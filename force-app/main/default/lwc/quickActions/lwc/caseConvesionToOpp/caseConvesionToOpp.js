...
export default class CaseConversion extends NavigationMixin(LightningElement) {
    ...
    wiredRecord({ error, data }) {
        if (error) {
            ...
            if (!getFieldValue(data, PPOJECT_ADDRESS_VALID_FIELD) && getFieldValue(data, PROJECT_NAME_FIELD) === null) {
                ...
                this.showError(constantUtils.CASE_CONVERSION_MESSAGES.WEB_FIELDS_ERROR_MESSAGE_OPP.replace('{0}', '"' + missedFieldsArr.join('" , "') + '"'));
                this.dispatchEvent(new CloseActionScreenEvent());
            } else {
                this.convertCase();
            }
    ...
    convertCase() {
        ...
        .then((leadId) => {
            this.showSuccess(constantUtils.CASE_CONVERSION_MESSAGES.SUCCESS_MESSAGE_OPP);
